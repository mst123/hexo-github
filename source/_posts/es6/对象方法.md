...
ES6 提出“Same-value equality”（同值相等）算法，用来解决这个问题。`Object.is`就是部署这个算法的新方法。它==用来比较两个值是否严格相等，与严格比较运算符（===）的行为基本一致==。
...
注意，如果==目标对象与源对象有同名属性，或多个源对象有同名属性，则**后面的属性会覆盖前面**的属性====。==
...
上面代码中，==两个对象合并的逻辑可以写成一个函数，包含get set函数==。
...
![propertiesObject](对象方法/objectDes.png) 
#### 3）创建空对象时不同
![propertiesObject](对象方法/objectNull.png) 
...
```
打印出 实例c 看看结构是怎样的
![propertiesObject](对象方法/object1.png) 
...
```
因为 Object.assing 是**不能拷贝到继承或原型上的方法的**。所以 实例c2 没有 a 这个属性。那要怎么要才能拷贝到原型上的方法呢？
- 第一种方法
```
...
[转载至-简书](https://www.jianshu.com/p/28d85bebe599)
