...
这里要求同名的类和命名空间在定义的时候，类的定义必须在命名空间前面，最后合并之后的效果，一个包含一些以命名空间导出内容为静态属性的类，来看例子：（有时候控制台打印结果并不直观）
（不太明白）
``` {.language-typescript}
class Validation {
    checkType() { }
...
function Validation(checkNumber, checkString, numberReg, stringReg){
    this.checkNumber = checkNumber
    this.checkString = checkString
    this.numberReg = numberReg
    this.stringReg = stringReg
}
*/
```
